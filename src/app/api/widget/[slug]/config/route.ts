...
export async function GET(req: NextRequest, { params }: { params: { slug: string } }) {
  try {
    await seedDb();
    const co = await queryOne('SELECT c.*, cs.base_rate_per_hour, cs.min_hours, cs.deposit_type, cs.deposit_amount, cs.mileage_rate, cs.google_maps_key FROM companies c LEFT JOIN company_settings cs ON cs.company_id = c.id WHERE c.slug = ?', [params.slug]);
