...
export async function POST(req: NextRequest, { params }: { params: { slug: string } }) {
  try {
    ...
    if (body.sqft) {
      const sqftFactor = body.sqft / 1500; // 1500 sqft as baseline
      eh = Math.max(eh, Math.round(eh * sqftFactor * 10) / 10);
    }
    if (body.fullness) {
      const fullnessFactor = body.fullness; // already 0.25 - 1.25
      eh = Math.round(eh * fullnessFactor * 10) / 10;
      ep = Math.round(ep * fullnessFactor);
    }

    const hourlyRate = Number(ru?.hourly_rate || st?.base_rate_per_hour || 150);
    ep = Math.max(ep, Math.round(hourlyRate * eh));

    const da = st?.deposit_type === 'percent'
      ? ep * (Number(st.deposit_amount) / 100)
      : (Number(st?.deposit_amount) || 100);
